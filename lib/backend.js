...
Backend.prototype.saveElisp = function(elisp, user, callback) {
    ...
    step(
        ...
        function(err) {callback(err, normalize(pkg))});
};
...
/**
 * Non-destructively normalize the properties in a Mongo record for external
 * consumption. This gets rid of internal attributes (starting with an
...
function normalize(obj) {
    var newObj = {};
    for (var prop in obj) {
        if (!prop.match(/^_/)) newObj[prop] = obj[prop];
    }
    if (obj._id) newObj.created = obj._id.generationTime;
    return newObj;
};
