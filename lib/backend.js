...
Backend.prototype.saveElisp = function(elisp, user, callback) {
    ...
    step(
        ...
        function(err) {
            if (err) throw err;
            self.write_(pkg.name + '.el', elisp, this);
        },
...
Backend.prototype.saveTar_ = function(pkg, tar, user, callback) {
    ...
    step(
        ...
        function(err) {
            ...
            self.write_(pkg.name + '.tar', tar, this);
        },
        function(err) {callback(err, pkg)});
};
...
Backend.prototype.savePackage_ = function(pkg, user, callback) {
    var self = this;
    step(
        function() {self.tryUpdate_(pkg, this)},
        function(err) {
            if (err) throw err;
            pkg._id = pkg.name.toLowerCase();
            if (_.isEmpty(pkg.owners)) {
                pkg.owners.push(user._id);
                self.users_.update(
                      {'_id': user._id}, {$push: {packages: pkg._id}}, this);
            } else if (_.include(pkg.owners, user._id)) {
                this();
            } else {
        ...
        function(err) {
            if (err) throw err;
            self.packages_.update({'_id': pkg._id}, pkg, {upsert: true},
                                  self.checkErrorCb(callback));
        }, callback);
};


/**
 * If the package already exists, modifies the object accordingly.
 * This means adding the owners and creation time of the old package.
 * If the package doesn't already exist, this adds an empty object for owners
 * and the current time for the creation time.
 *
 * @param {Object} pkg The package metadata.
 * @param {Function(Error=)} callback
 */
Backend.prototype.tryUpdate_ = function(pkg, callback) {
    var self = this;
    step(
        function() {
            self.packages_.findOne({'_id': pkg.name.toLowerCase()},
                                   ['owners', 'created'], this)
        },
        function(err, oldPkg) {
            if (err) throw err;
            ...
            if (!oldPkg) {
                pkg.owners = [];
                pkg.created = new Date();
                return null;
            }

            pkg.owners = oldPkg.owners;
            pkg.created = oldPkg.created;
            return null;
        }, callback);
};
