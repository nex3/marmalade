...
Backend.prototype.loadPackageData = function(name, version, type, callback) {
    var self = this;
    var key = {_name: key_(name), _version: versionIx_(version)};
    var pkg;
    step(
        function() {
        ...
        function(err, data) {
            if (err) callback(err);
                ...
                self.packages_.update({_name: key_(name)},
                                      {$inc: {downloads: 1}},
                                      _.identity);
...
Backend.prototype.loadPackage = function(name, callback) {
    this.packages_.findOne({_name: key_(name)}, callback)
};
...
Backend.prototype.loadPackageVersion = function(name, version, callback) {
    ...
    var pkg;
    step(
        function() {self.packages_.findOne({_name: key_(name)}, this)},
        function(err, pkg_) {
            if (err) throw err
            if (!pkg_) return callback(null, null, null);
            pkg = pkg_;
            self.packageVersions_.findOne({
                _name: key_(name), _version: versionIx_(version)
            }, this);
        },
...
Backend.prototype.addPackageOwner = function(
    ...
    step(
        function() {
            ...
            self.users_.findOne({_name: key_(newOwnerName)},
...
Backend.prototype.removePackageOwner = function(
    ...
    step(
        function() {
            ...
            var unset = {};
            unset['owners.' + key_(removedOwnerName)] = 1;
            self.packages_.update(
                  {_name: pkgKey}, {$unset: unset}, this.parallel());

            self.users_.update({_key: key_(removedOwnerName)},
...
Backend.prototype.searchPackages = function(query, opt_fields, opt_opts) {
    ...
    return this.packageStream(
        {_name: new RegExp(util.regexpEscape(key_(query)))},
        opt_fields, opt_opts);
};
...
Backend.prototype.loadUser = function(name, password, callback) {
    var self = this;
    step(
        function() {self.users_.findOne({_name: key_(name)}, this)},
        function(err, user) {
            if (err) throw err;
            if (!user) return null;
...
Backend.prototype.loadPublicUser = function(name, callback) {
    var self = this;
    self.users_.findOne({_name: key_(name)}, {
...
Backend.prototype.resetPassword = function(name, callback) {
    ...
    step(
        function() {
            self.users_.findOne({_name: key_(name)}, this.group()());
