...
Backend.prototype.savePackage_ = function(pkg, user, callback) {
    ...
    step(
        function() {
            self.packages_.findOne({'_id': pkg._name}, ['owners'], this);
        },
        function(err, oldPkg) {
            if (err) throw err;
            if (!oldPkg) {
                self.saveNewPackage_(pkg, user, this);
            } else if (_.include(oldPkg.owners, user._id)) {
                this();
            } else {
        ...
        function(err) {
            if (err) throw err;
            self.packageVersions_.insert(pkg, self.checkErrorCb(callback));
        }, callback);
};
