...
Backend.prototype.loadPackageData = function(name, version, type, callback) {
    var self = this;
    var key = {_name: name.toLowerCase(), _version: versionIx_(version)};
    var pkg;
    step(
        function() {
        ...
        function(err, data) {
            if (err) callback(err);
                ...
                self.packages_.update({_name: name.toLowerCase()},
                                      {$inc: {downloads: 1}},
                                      _.identity);
...
Backend.prototype.loadPackage = function(name, callback) {
    this.packages_.findOne({_name: name.toLowerCase()}, callback)
};
...
Backend.prototype.loadPackageVersion = function(name, version, callback) {
    ...
    var pkg;
    step(
        function() {self.packages_.findOne({_name: name.toLowerCase()}, this)},
        function(err, pkg_) {
            if (err) throw err
            if (!pkg_) return callback(null, null, null);
            pkg = pkg_;
            self.packageVersions_.findOne({
                _name: name.toLowerCase(), _version: versionIx_(version)
            }, this);
        },
...
Backend.prototype.addPackageOwner = function(
    ...
    var pkgKey = pkgName.toLowerCase();
    var self = this;
    step(
        function() {
            ...
            this.users_.findOne({_name: newOwnerName.toLowerCase()},
...
Backend.prototype.searchPackages = function(query, opt_fields, opt_opts) {
    ...
    return this.packageStream(
        {_name: new RegExp(util.regexpEscape(query.toLowerCase()))},
        opt_fields, opt_opts);
};
