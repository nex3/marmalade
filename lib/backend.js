...
Backend.prototype.write_ = function(filename, data, callback) {
    this.grid_(filename, 'w', _.bind(function(err, grid) {
        this.writeHelper_(grid, data, callback, err);
    }, this));
};

/**
 * Write a single chunk to the MongoDB GridStore. These chunks need to be
 * separated manually because aparently the mongo library doesn't do it
 * automatically.
 *
 * @param {!mongodb.GridStore} grid The grid object.
 * @param {string} data The data to write.
 * @param {function(Error=)} callback
 * @param {Error=} err
 */
Backend.prototype.writeHelper_ = function(grid, data, callback, err) {
    try {
        if (err) callback(err);
        else if (data.length <= grid.chunkSize) {
            grid.write(data, true, callback);
        } else {
            var prefix = data.slice(0, grid.chunkSize);
            var suffix = data.slice(grid.chunkSize, data.length);
            grid.write(prefix, false,
                       _.bind(this.writeHelper_, this, grid, suffix, callback));
        }
    } catch (e) {
        callback(e);
    }
        ...
        function(err, data) {
            if (err) callback(err);
                ...
                self.packages_.update({_name: self.key(name)},
                                      {$inc: {downloads: 1}},
                                      _.identity);
                callback(null, new Buffer(data, 'binary'), pkg);
            }
        });
};
...
Backend.prototype.saveTarFile = function(file, user, callback) {
    ...
    step(
        function() {
            try {
                packageParser.parseTarFile(file, this)
            } catch (e) {
                console.log("1: " + e);
                exit(1);
            }
...
Backend.prototype.removePackageVersion = function(name, version, callback) {
    ...
    step(
        ...
        function(err, pkg) {
            if (err) throw err;
            this.grid_(pkgFile_(key, pkg.type, versionIx_(version)), this);
        },
        function(err, grid) {
            if (err) throw err;
            grid.deleteChunks(this);
        },
        function(err) {
 ...
 *   if the username or token was invalid.
 */
