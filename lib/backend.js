...
Backend.prototype.addPackageOwner = function(
    ...
    step(
        function() {
            ...
            this.users_.findOne({_name: key_(newOwnerName)},
                                {fields: ['name', 'email']}, this);
        },
        function(err, newOwner) {
            if (err) throw err;
            if (!user) throw exports.InputError('No user named ' + owner);

            var set = {};
            set['owners.' + newOwner._name] = newOwner.email;
            this.packages_.update(
                  {_name: pkgKey}, {$set: set}, this.parallel());

            this.users_.update({_id: newOwner._id},
