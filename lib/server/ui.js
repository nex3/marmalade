...
exports.create = function(be) {
     ...
     * Search for packages.
     */
    app.get('/packages/search', function(req, res, next) {
        var query;
        var page;
        step(
            function() {
                query = req.requiredParam('q');
                page = Number(req.param('page')) || 0;
                page = Math.max(Math.floor(page), 0);
                stream.all(be.searchPackages(query, [], {
                    skip: page * PACKAGES_PER_PAGE,
                    limit: PACKAGES_PER_PAGE + 1
                }), this);
            },
            function(err, packages) {
                ...
                var hasNextPage = !!packages[PACKAGES_PER_PAGE];
                if (hasNextPage) packages.pop();
                res.render('packages/search.jade', {
                    locals: {
                        query: query,
                        page: page,
                        hasNextPage: hasNextPage,
